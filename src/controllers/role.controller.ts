...
import { Controller, Delete, Get, Post, Put, RequirePermission } from "../decorators";
...
export class RoleController {
    ...
    async assignPermissions(req: Request, res: Response) {
        ...
        if (!Array.isArray(permissionIds)) return res.status(400).json({ message: "permissionIds必须为数组" });
        try {
